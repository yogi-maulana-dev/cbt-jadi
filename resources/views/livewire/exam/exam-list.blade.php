<div wire:poll.10s>
    ...
    @else
        <div class="grid gap-4 sm:grid-cols-2">
            @foreach ($this->exams as $test)
                @php($attempt = $this->attempts->get($test->id))
                <div class="flex flex-col rounded-xl bg-white p-5 shadow" wire:key="exam-{{ $test->id }}">
                            ...
                            <div class="flex items-end gap-2">
                                @if ($test->token)
                                    <div>
                                        ...
                                        <input type="text" wire:model="tokens.{{ $test->id }}"
                                               class="mt-1 w-32 rounded-md border-gray-300 text-sm shadow-sm">
                                    </div>
                                @endif
                                <button type="button" wire:click="start({{ $test->id }})"
